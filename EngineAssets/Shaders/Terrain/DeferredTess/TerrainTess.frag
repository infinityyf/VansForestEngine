...
layout(location = 3) in vec2 inNoiseGradient;  // 来自 TES 的噪声梯度
layout(location = 4) in vec4 motionCurrentClip;
layout(location = 5) in vec4 motionPreviousClip;
...
vec3 CalculateFinalNormal(vec2 uv, vec2 noiseGradient) {
    ...
    // 高度场 H(x,z) = H_heightmap(x,z) + noise(x,z)，斜率扰动方向取负梯度。
