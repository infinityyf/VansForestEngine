...
namespace Vans::EditorAPI
{
	...
	struct ReflectionProbeEntrySnapshot
	{
		...
		bool editable = true;
		std::uint32_t textureResolution = 0;
	...
	struct ReflectionProbeSettingsSnapshot
	{
		...
		std::uint32_t coveredReceiverCount = 0;
		float coveredSurfaceFraction = 0.0f;
		float reachableSurfaceFraction = 0.0f;
		std::string placementError;
		std::uint32_t texturePageCount = 0;
		std::uint32_t pendingCaptureCount = 0;
		std::uint64_t residentTextureBytes = 0;
	...
	struct GIProbePlacementSettingsSnapshot
	{
		bool enabled = false;
		float minProbeSpacing = 0.5f;
		float maxProbeSpacing = 4.0f;
		float parentProbeMaxSize = 5.0f;
		std::uint32_t maxProbeCount = 65536u;
		std::uint32_t maxProbeUpdatesPerFrame = 4096u;
	...
	{
		bool available = false;
		std::uint32_t regionStableId = 0;
		std::uint32_t physicalProbeCount = 0;
	...
	struct SkyLightingSettings { float intensity = 1.0f; };

	struct EnvironmentSettings
	{
        SkyLightingSettings skyLighting;
