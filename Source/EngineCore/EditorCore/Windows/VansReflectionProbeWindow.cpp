...
void VansReflectionProbeWindow::ShowWindow(Vans::EditorAPI::IEngineEditorAPI& editorAPI)
{
	...
	Vans::EditorAPI::IReflectionProbeEditorAPI& probeAPI = editorAPI;
	if (!VansEditorWindow::IsWindowOpen(VansEditorWindowId::ReflectionProbe))
		return;

	if (!ImGui::Begin("Reflection Probe Inspector", VansEditorWindow::WindowOpenState(VansEditorWindowId::ReflectionProbe)))
	{
		ImGui::End();
		return;
	}

	Vans::EditorAPI::ReflectionProbeSettingsSnapshot settings = probeAPI.GetReflectionProbeSettings();
	...
	if (ImGui::CollapsingHeader("Placement", ImGuiTreeNodeFlags_DefaultOpen))
	{
		...
		if (ImGui::Button("Apply Placement"))
		{
			...
			const bool applied = probeAPI.ApplyReflectionProbeSettings(settings); changed = false;
			settings = probeAPI.GetReflectionProbeSettings();
		...
		{
			probeAPI.GenerateAutoReflectionProbes();
			settings = probeAPI.GetReflectionProbeSettings();
		}
		ImGui::EndDisabled();
		ImGui::SameLine();
		...
		{
			probeAPI.ClearAutoReflectionProbes(); settings = probeAPI.GetReflectionProbeSettings();
	...
	if (ImGui::CollapsingHeader("Bake", ImGuiTreeNodeFlags_DefaultOpen))
	{
		...
		if (ImGui::Button("Request Bake All"))
			probeAPI.RequestReflectionProbeBakeAll();
		ImGui::SameLine();
		if (ImGui::Button("Bake Queue Now"))
			probeAPI.BakeQueuedReflectionProbesNow();
		ImGui::SameLine();
		if (ImGui::Button("Store Runtime Config in Scene"))
			probeAPI.SaveReflectionProbeConfiguration();
	}
			...
			&& settings.editor.selectedProbeIndex < static_cast<int>(settings.probes.size()))
		{
			...
			if (probe.type != 2)
			{
				if (ImGui::Button("Request Bake Selected"))
				{
					if (changed)
					{
						probeAPI.ApplyReflectionProbeSettings(settings);
						changed = false;
					}
					probeAPI.RequestReflectionProbeBake(static_cast<std::uint32_t>(selectedIndex));
				}
				ImGui::SameLine();
				if (probe.autoGenerated && ImGui::Button("Convert To Manual"))
				{
					...
					probeAPI.ConvertReflectionProbeToManual(static_cast<std::uint32_t>(selectedIndex));
					settings = probeAPI.GetReflectionProbeSettings();
				}
			}
			...
			if (probe.type != 2 && ImGui::CollapsingHeader("Cubemap Preview", ImGuiTreeNodeFlags_DefaultOpen))
			{
				...
				if (settings.editor.previewCubemap)
				{
					...
					std::vector<Vans::EditorAPI::RenderTexturePreview> previews =
						renderAPI.QueryRenderTexturePreviews(filter);
