...
const char* ToString(EditorAPI::AssetType assetType)
{
	...
	case EditorAPI::AssetType::IESProfile: return "IESProfile";
...
EditorAPI::AssetType EditorAssetTypeFromString(const std::string& value)
{
	...
	if (type == "iesprofile") return EditorAPI::AssetType::IESProfile;
...
bool ValidateDocumentPropertyPointer(
    const std::string& propertyPointer,
    bool allowRoot,
    std::string* error,
    const char* addressName)
{
    if (propertyPointer.empty())
    {
        if (allowRoot)
            return true;
        if (error)
            *error = std::string(addressName) + " must be a non-root JSON Pointer";
        return false;
    }
    if (propertyPointer.front() != '/')
    {
        if (error)
            *error = std::string(addressName) + " must be a non-root JSON Pointer";
        return false;
    }
    if (!IsValidPropertyPointerSyntax(propertyPointer))
    {
        if (error)
            *error = std::string(addressName) + " contains an invalid JSON Pointer escape";
        return false;
    }
    return true;
}
