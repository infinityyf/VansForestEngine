...
bool VansModelLodBuilder::Build(VansAssetDatabase& database,
    const std::vector<VansModelLodSourcePart>& sources,const VansModelLodSettings& settings,
    VansModelLodBuildMode mode,VansModelLodAsset& result,std::string& error)
{
    error.clear();
    try {
        bool validRatios=settings.ratios.size()>=MinimumModelLodLevelCount&&settings.ratios.size()<=MaximumModelLodLevelCount;
        float previousRatio=1;
        for(float ratio:settings.ratios){
            if(!std::isfinite(ratio)||ratio<=0||ratio>=previousRatio){validRatios=false;break;}
            previousRatio=ratio;
        }
        if(sources.empty() || sources.size()>64 || !std::isfinite(settings.maximumError) || settings.maximumError<=0 || settings.maximumError>.25f || !validRatios) {error="Invalid model LOD ratios or error limit.";return false;}
        VansScopedIOContext io(VansIODomain::Authoring,"Authoring.BuildModelLod",true);
        const bool publish=mode==VansModelLodBuildMode::Publish;
        ...
        for(size_t i=0;i<sources.size();++i){const auto& source=sources[i];
            ...
            const float scale=meta.ReadFloatSetting("scaleFactor",1.f);
        ...
        const auto location=VansDerivedArtifactLayout::ModelLodAuthoringAssetSet(database.AssetsRoot(),key);
        if(!location||location.artifactClass!=VansDerivedArtifactClass::AuthoringAssetSet)
            throw std::runtime_error("Invalid model LOD artifact location.");
        const auto& directory=location.path;
        ...
        if(cached)built=ReadResult(manifest.at("result"));
            ...
            const auto stage=[&](const std::string& name,const std::string& data){
                if(publish){VansStagedFile file;if(!VansFileStorage::StageWriteBytes(directory/name,data,file,error))throw std::runtime_error(error);
                    transaction.Add(std::move(file));}
                files.push_back({{"name",name},{"hash",ComputeMemoryFnv1a64(data.data(),data.size())}});
            };
            ...
            if(publish&&!transaction.Publish(error))return false;
        }
        // 先完整发布不可变产物，再注册引用；调用者最后更新自己的作者文档。
        if(publish)for(const auto& file:manifest.at("files")){
