...
{
	auto* storage = world.FindStorage<VansRuntimeAnimationComponent>(
...
struct VansAnimationRestoreState
{
...
class VansAnimationTimelineApplier final : public IVansTimelineOutputApplier
{
public:
	VansAnimationTimelineApplier(VansRuntimeWorld& world,
		...
		const VansResolvedTimelineTarget& target, VansTimelineOutputPayloadView view) override
	{
		...
		if (!sample->active)
		{
			if (VansAnimationRestoreState* state = m_State.ResolveWriter(context.writer))
		...
		auto [restore, state] = m_State.Acquire(context.writer, [&]
		{
			...
			if (!maskGuid.empty())
			{
				...
				if (!VansAssetGuid::TryParse(maskGuid, guid))
					return VansAnimationRestoreState{ context.writer, component, {} };
				const auto mask = m_Repository.ResolveLatest<VansBoneMaskAsset>(guid);
				if (!mask) return VansAnimationRestoreState{ context.writer, component, {} };
				VansCompiledBoneMask compiled = VansBoneMaskCompiler::Compile(*mask, runtime->animationNode->GetSkeleton());
				if (!compiled.valid) return VansAnimationRestoreState{ context.writer, component, {} };
	...
	{
		VansAnimationRestoreState* state = m_State.Resolve(token.handle);
...
private:
	...
	VansTimelineModuleApplierState<VansAnimationRestoreState> m_State;
};

struct VansParameterRestoreState
{
	VansTimelineWriterHandle writer;
	VansComponentHandle component;
	std::string name;
...
class VansAnimatorParameterTimelineApplier final : public IVansTimelineOutputApplier
{
public:
	explicit VansAnimatorParameterTimelineApplier(VansRuntimeWorld& world) : m_World(world) {}
	VansTimelineOutputTypeId OutputType() const override
		...
		const VansResolvedTimelineTarget& target, VansTimelineOutputPayloadView view) override
	{
		...
		auto [restore, state] = m_State.Acquire(context.writer, [&]
		{ return VansParameterRestoreState{ context.writer, component, name, found->second }; });
		(void)state;
	...
	bool Restore(VansTimelineRestoreToken token) override
	{
		VansParameterRestoreState* state = m_State.Resolve(token.handle);
		if (!state) return false;
		if (auto* runtime = ResolveAnimation(m_World, state->component))
			if (auto* controller = runtime->animationNode ? runtime->animationNode->GetController() : nullptr)
...
private:
	VansRuntimeWorld& m_World;
	VansTimelineModuleApplierState<VansParameterRestoreState> m_State;
};

struct VansBoneRestoreState
{
	VansTimelineWriterHandle writer;
	VansComponentHandle component;
	std::string bone;
...
class VansBoneOverrideTimelineApplier final : public IVansTimelineOutputApplier
{
public:
	explicit VansBoneOverrideTimelineApplier(VansRuntimeWorld& world) : m_World(world) {}
	VansTimelineOutputTypeId OutputType() const override
		...
		const VansResolvedTimelineTarget& target, VansTimelineOutputPayloadView view) override
	{
		...
		auto [restore, state] = m_State.Acquire(context.writer, [&]
		{
			VansBoneRestoreState result; result.writer = context.writer; result.component = component; result.bone = bone;
		...
		if (!ikBinding.empty())
		{
			...
			const int boneIndex = skeleton.FindBoneIndex(bone);
			if (!ikTarget || !Vans::VansTransformStore::IsAllocated(targetTransform) ||
				!Vans::VansTransformStore::IsAllocated(ownerTransform) || !controller ||
				boneIndex < 0)
				return { VansTimelineApplyStatus::Failed, {}, "Bone Override IK target is not live for the bound skeleton" };
			Vans::VansTransform owner = Vans::VansTransformStore::Read(ownerTransform);
			const glm::mat4 ownerInverse = glm::inverse(owner.GetModelMatrix());
			const glm::vec3 targetModel = glm::vec3(ownerInverse * glm::vec4(
				Vans::VansTransformStore::Read(targetTransform).m_Position, 1.0f));
	...
	bool Restore(VansTimelineRestoreToken token) override
	{
		VansBoneRestoreState* state = m_State.Resolve(token.handle);
		if (!state) return false;
		if (auto* runtime = ResolveAnimation(m_World, state->component))
...
private:
	VansRuntimeWorld& m_World;
	VansTimelineModuleApplierState<VansBoneRestoreState> m_State;
};
}
...
{
	if (!registry.Register(std::make_shared<VansAnimationTimelineApplier>(
		world, repository), error)) return false;
	if (!registry.Register(std::make_shared<VansAnimatorParameterTimelineApplier>(world), error)) return false;
	return registry.Register(std::make_shared<VansBoneOverrideTimelineApplier>(world), error);
}
}
