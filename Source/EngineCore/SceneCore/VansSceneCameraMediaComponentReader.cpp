...
std::optional<std::string> ReadAssetGuid(
	const VansSerializedValue& object,
	const VansSceneAssetGuidResolver& assetGuidResolver)
{
	const VansSerializedValue* source = FindObjectField(object, "source");
	if (!source || !assetGuidResolver)
		return std::nullopt;

	const std::string assetGuid = assetGuidResolver(*source);
	return assetGuid.empty() ? std::nullopt : std::optional<std::string>(assetGuid);
}
}
	...
	const VansSceneAssetGuidResolver& assetGuidResolver)
{
	...
	if (std::optional<std::string> assetGuid = ReadAssetGuid(audioNode, assetGuidResolver))
		config.assetGuid = *assetGuid;
	...
	const VansSceneAssetGuidResolver& assetGuidResolver)
{
	...
	VansSceneVideoComponentConfig config;
	if (std::optional<std::string> assetGuid = ReadAssetGuid(videoNode, assetGuidResolver))
		config.assetGuid = *assetGuid;
	return config;
}
}
