...
// When compiling extension sources as static objects (not dllimport from the
// NvCloth DLL), NV_CLOTH_IMPORT must be empty to allow function definitions.
#undef  NV_CLOTH_IMPORT
...
// ClothClone.h is an internal NvCloth src file not distributed with the SDK
// and is not used by any code in this extensions file - omit it.
...
// safeLog2: internal helper from ClothImpl.h (not distributed in the public SDK).
// Returns -1e37 for zero input; uses std::log2 for portability.
static inline float safeLog2(float x)
{
    return x <= 0.0f ? -1e37f : std::log2(x);
}

// LogInvalidParameterFn: internal Callbacks.cpp function not exported from the DLL.
// Stub: forward the message to stderr so validation failures are still visible.
void LogInvalidParameterFn(const char* fileName, int lineNumber, const char* msg, ...)
{
    va_list args;
    va_start(args, msg);
    fprintf(stderr, "[NvCloth] Invalid parameter at %s(%d): ", fileName, lineNumber);
